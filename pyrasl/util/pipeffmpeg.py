...
class InputVideoStream:
    ...
    def open(self, path):
        ...
        self.p = sp.Popen(
            ...
            #stderr=sp.PIPE,   # This seems to cause a deadlock when uncommented:
...
class OutVideoStream:
    ...
    def open(self, path):
        ...
        cmd = [
            ...
            '-f', self.oformat,
            '-r', str(self.oframerate),
            self.filepath,
        ]
        if self.verbose:
            print(' '.join([str(x) for x in cmd]))
            self.p = sp.Popen(cmd, stdin=sp.PIPE, shell=False)
        else:
            devnull = open('/dev/null', 'w')
            self.p = sp.Popen(cmd, stdin=sp.PIPE, shell=False, stderr=devnull,
                    stdout=devnull)
        # self.p = sp.Popen(
        #     cmd,
        #     stdin=sp.PIPE,
    ...
    def writeframe(self, frameraw):
        self.p.stdin.write(frameraw.tostring(order='c'))
...
def create_h264_mp4(filename, im_shape, fps=30, ipix_fmt='gray',
                    opix_fmt='yuv420p', verbose=True):
    '''
    ...
    video_writer = OutVideoStream(verbose)
    video_writer.opix_fmt = opix_fmt
    video_writer.ipix_fmt = ipix_fmt
    video_writer.oformat = 'mp4'
    video_writer.bitrate = '1500k'
    video_writer.iframerate = fps
    video_writer.oframerate = fps

    # Make sure the output video size is a power of two in each dimension
    width = im_shape[1]
    height = im_shape[0]
    if width % 2 != 0:
        width += 1
    ...
    video_writer.isize = '%dx%d' % (im_shape[1], im_shape[0])
    video_writer.osize = '%dx%d' % (width, height)
    video_writer.ocodec = 'libx264'
    if verbose:
