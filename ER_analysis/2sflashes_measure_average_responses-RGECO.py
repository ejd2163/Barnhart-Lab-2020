...
for br in brain_regions[:]:
    ...
    for directory,p,row in zip(dir_list[:],path_list[:],rows[:]):
        ...
        image_dir = directory+'/RGECO'
        plot_dir = directory+'/plots/'
        m_dir = directory+'/measurements/'     
        imagefiles = glob.glob(os.path.join(image_dir,'*tif'))
        #imagefiles is a list of all image frames (0000,0001,0002,etc.)

        
        mask_file = directory+'/mask-'+br+'.tif'
        
        #I is going to be an array of all image frames (ea/ image frame contains 512x320 pixel values)
        ...
        m = labels[0]==1
        IBG = I*m #multiply each image frame to the mask of the background
