...
out_mdir = MLB_dir+'/measurements/10s'
map_mdir = out_mdir+'/mappable/'
unmap_mdir = out_mdir+'/unmappable/'

plot_dir = MLB_dir+'/plots/10s'
...
for br in brain_regions[:]:
    ...
    # # #RGECO gratings responses data
    # # norm_cytg = []
    # # abn_cytg=[]
    # # g_header, rows = datafile.DataFile(gratings_mdir+'/average_DFF-'+br+'-RGECO.csv').get_header_and_data()
    # # g_cyt = numpy.asarray(rows)
    # # print('gratings count: '+str(len(g_cyt)))
    
    # # #ER210 gratings responses data
    # # norm_ERg = []
    # # abn_ERg=[]
    # # g_header, rows = datafile.DataFile(gratings_mdir+'/average_DFF-'+br+'-ER210.csv').get_header_and_data()
    # # g_ER = numpy.asarray(rows)    
    ...
    #     #check if each pair of epochs reaches threshold
    #     # rmax_pair=0
    #     # if numpy.mean(rmax[:2])>=threshold and numpy.mean(rmax[2:])>=threshold:
    #     #     rmax_pair=True
    #     # else: 
    #     #     rmax_pair=False
    ...
    #         # norm_cytg.append(g_cyt[g,:]) #gratings RGECO responses
    #         # norm_ERg.append(g_ER[g,:]) #gratings ER responses
    ...
    #         # abn_cytg.append(g_cyt[g,:])
    #         # abn_ERg.append(g_ER[g,:])  
    ...
    # # datafile.write_data_file([g_header]+norm_cytg,map_mdir+'/average_DFF-'+br+'-RGECO.csv') #gratings responses .csv RGECO
    # # datafile.write_data_file([g_header]+norm_ERg,map_mdir+'/average_DFF-'+br+'-ER210.csv') #gratings responses .csv ER210
    ...
    # # datafile.write_data_file([g_header]+abn_cytg,unmap_mdir+'/average_DFF-'+br+'-RGECO.csv') #gratings responses .csv RGECO
    # # datafile.write_data_file([g_header]+abn_ERg,unmap_mdir+'/average_DFF-'+br+'-ER210.csv') #gratings responses .csv ER210
    ...
    flash_header,rows = datafile.DataFile(flash_mdir+'/average_responses-'+br+'-RGECO.csv').get_header_and_data()
    flash_cyt = numpy.asarray(rows)
    print('flash count: '+str(len(flash_cyt)))
    ...
    count_map=0
    count_unmap=0
    while n<len(R) and f<len(flash_cyt):  #and g<len(g_cyt)
    #while n==8:
        # print(R[n,2]) #ROI number
        o = list(R[n,:5]) #sample, br, ROI, cell_x, cell_y
        o2 = list(R[n,:5])
